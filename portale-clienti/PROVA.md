...
3. Alla prima entrata spunta «Ho letto l’informativa privacy» e «Ricorda la presa visione». L’amministratore di prova usa dati fittizi ed esiste solo nel database Codespaces. Nel terminale integrato del Codespace (container **app**) esegui `cat /private-documents/.demo-admin-login`. Se vedi solo le credenziali amministratore, prima esegui `cd /workspace/portale-clienti && php tests/demo-seed.php`, poi ripeti `cat /private-documents/.demo-admin-login`. Se invece stai usando un terminale esterno al container, dalla radice del repository esegui `docker compose -f .devcontainer/docker-compose.yml exec -T app cat /private-documents/.demo-admin-login`. Il file è privato e non è nel repository. La password viene generata una volta e resta la stessa nei riavvii della demo. Nello stesso file privato trovi anche le credenziali **Operatore** e **Cliente** per confrontare le tre viste:
