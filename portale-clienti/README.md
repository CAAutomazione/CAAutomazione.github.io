...
Per preparare il trasferimento al server leggi [TOPHOST.md](TOPHOST.md). Verifica prima HTTPS e isolamento della cartella pubblica.
...
5. Attivare HTTPS. Creare il primo amministratore tramite il modulo di attivazione protetto descritto in [TOPHOST.md](TOPHOST.md), oppure da CLI con `php bin.php admin CA Automazione caniatoa@libero.it`. La password va scelta nuova e non inserita nel repository.
6. Configurare l'invio periodico delle email tramite la chiamata POST protetta a `/?action=jobs` descritta in [TOPHOST.md](TOPHOST.md), oppure tramite un cron CLI che esegua `php /percorso/portale-clienti/bin.php jobs` ogni 5 minuti. Non rendere pubblico `bin.php`.
...
La scheda **Impostazioni** dell’amministratore permette di impostare il numero massimo di download per documento e i giorni di accesso dalla data di caricamento. `0` lascia la regola illimitata. Ogni documento conserva i limiti in vigore al momento del caricamento: cambiare le impostazioni non modifica quelli già consegnati. Quando un limite è raggiunto, il cliente vede ancora la voce nel proprio elenco ma non può aprire o scaricare il file. Amministratore e operatore possono togliere o ripristinare manualmente l'accesso del cliente a un documento; il ripristino manuale prevale sui limiti di quel documento, e resta in vigore finché non viene disattivato. I download concorrenti vengono contati sotto blocco del record. La gestione tecnica di SMTP, dimensione massima del caricamento, spazio e URL resta nel file `config.php` sul server.

Il database di prova Codespaces è separato dalla produzione: usa un amministratore fittizio con password casuale conservata nel container Codespaces in `/private-documents/.demo-admin-login` (nel terminale del Codespace: `cat /private-documents/.demo-admin-login`), cinque aziende, dieci clienti e documenti inventati. Non importare la banca dati di prova sul server reale. Per aggiornare un database reale precedente è necessario applicare in ordine `sql/003-document-settings.sql` e `sql/004-document-access.sql` dopo `sql/002-operator-role.sql`, con backup preventivo. La migrazione 004 registra sui file preesistenti i limiti attualmente in vigore; eseguirla prima di cambiarli.
