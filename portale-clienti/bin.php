...
if($cmd==='jobs'){C::exec('DELETE FROM login_attempts WHERE occurred_at<UTC_TIMESTAMP()-INTERVAL 2 DAY');C::exec('DELETE FROM password_reset_tokens WHERE created_at<UTC_TIMESTAMP()-INTERVAL 2 DAY');\Portal\Notifications::quota();\Portal\Notifications::sendPending();echo "Controlli completati.\n";exit;}
