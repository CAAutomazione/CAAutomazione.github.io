...
$limits=$pdo->query("SHOW TABLES LIKE 'request_limits'")->fetch();
if (!$limits) foreach (explode("\n",file_get_contents(dirname(__DIR__).'/sql/005-request-limits.sql')) as $line) { $line=trim($line); if ($line!==''&&!str_starts_with($line,'--'))$pdo->exec($line); }
