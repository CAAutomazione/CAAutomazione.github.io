...
C::exec('INSERT INTO plants(company_id,name) VALUES(?,?)',[$a,'Impianto A']);$pa=(int)$pdo->lastInsertId();C::exec('INSERT INTO plants(company_id,name) VALUES(?,?)',[$b,'Impianto B']);$pb=(int)$pdo->lastInsertId();
foreach([[$a,'Alice','Cliente','alice@example.invalid','client',$pa],[$b,'Bob','Cliente','bob@example.invalid','client',$pb]] as [$company,$first,$last,$email,$role,$plant]){C::exec('INSERT INTO users(company_id,first_name,last_name,email,password_hash,role) VALUES(?,?,?,?,?,?)',[$company,$first,$last,$email,password_hash('test-password-1234',PASSWORD_DEFAULT),$role]);$id=(int)$pdo->lastInsertId();C::exec('INSERT INTO user_plants(user_id,plant_id) VALUES(?,?)',[$id,$plant]);if($company===$a)$alice=$id;}
