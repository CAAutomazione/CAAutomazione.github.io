...
final class Core {
 ...
 public static function policy(): array {static $policy; if($policy===null){$policy=['max_downloads'=>0,'retention_days'=>0];foreach(self::all("SELECT setting_key,value_int FROM portal_settings WHERE setting_key IN ('max_downloads','retention_days')") as $row)$policy[$row['setting_key']]=(int)$row['value_int'];}return $policy;}
 public static function available(array $document): bool {if($document['manual_access']!==null)return (bool)$document['manual_access'];if((int)$document['retention_days']>0 && strtotime($document['created_at'].' UTC') + (int)$document['retention_days']*86400 <= time())return false;if((int)$document['max_downloads']>0 && (int)($document['download_count']??0)>=(int)$document['max_downloads'])return false;return true;}
