...
final class Core {
 ...
 public static function available(array $document): bool {$policy=self::policy();if($policy['retention_days']>0 && strtotime($document['created_at'].' UTC') + $policy['retention_days']*86400 <= time())return false;if($policy['max_downloads']>0 && (int)($document['download_count']??0)>=$policy['max_downloads'])return false;return true;}
