...
final class Core {
 ...
 public static function session(): void {if(session_status()===PHP_SESSION_ACTIVE)return; if (PHP_SAPI !== 'cli' && empty($_SERVER['HTTPS']) && !in_array($_SERVER['REMOTE_ADDR']??'', ['127.0.0.1','::1'],true)) throw new RuntimeException('HTTPS richiesto');session_name('ca_portal');session_set_cookie_params(['lifetime'=>0,'path'=>'/','secure'=>!empty($_SERVER['HTTPS']),'httponly'=>true,'samesite'=>'Lax']);session_start();}
 public static function user(): array|false {self::session();if(empty($_SESSION['uid']))return false;$u=self::row('SELECT id,company_id,first_name,last_name,email,role,active,session_version FROM users WHERE id=?',[(int)$_SESSION['uid']]);if(!$u||!$u['active']||(int)$u['session_version']!==(int)($_SESSION['session_version']??-1)){self::logout();return false;}return $u;}
