...
Il grafico mostra una **stima di visitatori distinti per giorno**, non il numero certificato di persone: più dispositivi o persone dietro lo stesso IP possono modificare il risultato. Il server usa un'impronta giornaliera derivata da IP e browser, cancellata durante la manutenzione del pannello per i giorni precedenti agli ultimi due; la statistica aggregata e il registro accessi hanno una conservazione configurabile di 30, 90 o 180 giorni. La raccolta non usa cookie analytics nel browser. Il pannello usa il cookie tecnico di sessione (`ca_admin`), descritto nella cookie policy pubblicata.
